  ...
  return (
    <DeviceFrame>
      <div className="screen bg-white h-full flex flex-col overflow-hidden">
        ...
        <div className="auth-body px-6 py-5 flex-1 overflow-y-auto">
          <div className="auth-eyebrow text-[11px] font-bold text-teal tracking-[0.06em] uppercase mb-1.5">Welcome back</div>
          <h1 className="auth-heading font-head text-[26px] font-extrabold text-dark tracking-tight leading-[1.15] mb-1">Sign in to Zumi</h1>
          <p className="auth-sub text-sm text-slate-600 mb-6 leading-relaxed">Your community is waiting for you.</p>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="form-group">
              <div className="flex justify-between items-center mb-1.5">
                <label className="form-label text-[12px] font-semibold text-slate-600 tracking-[0.04em] uppercase mb-0">Password</label>
                <button type="button" className="text-[12px] text-teal font-medium hover:underline">Forgot?</button>
              </div>
              <div className="input-icon-wrap relative">
            ...
            <button 
              type="submit"
              disabled={isLoading}
              className={`btn btn-primary w-full p-[15px] rounded-r12 bg-teal text-white font-semibold text-[15px] transition-all flex items-center justify-center gap-2 mt-2 ${isLoading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-teal-dark active:translate-y-0'}`}
            >
              {isLoading ? <div className="spinner w-4.5 h-4.5 border-[2.5px] border-white/30 border-t-white rounded-full animate-spin"></div> : "Sign in"}
            </button>
          </form>

          <div className="divider-row flex items-center gap-3 my-4 mt-8">
            <div className="flex-1 h-[1px] bg-slate-100"></div>
            <span className="text-[12px] text-slate-400 font-medium lowercase">or</span>
            <div className="flex-1 h-[1px] bg-slate-100"></div>
          </div>
          ...
          <div className="switch-row text-center text-sm text-slate-600 pt-4 pb-4">
            Don&apos;t have an account? <Link href="/auth/signup" className="text-teal font-bold hover:underline">Sign up</Link>
          </div>
        </div>
      </div>
    </DeviceFrame>
